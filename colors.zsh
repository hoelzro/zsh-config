...
function {
  local ftcs_prompt=$'\e]133;A\e\\'
  local ftcs_command_start=$'\e]133;B\e\\'
  PS1="%{$ftcs_prompt%}\$([[ \$__vi_mode == 'vicmd' ]] && echo %U)%(2j.(%j⚙) .%(1j.(1⚙) .))\$(__vcs_prompt)%F{cyan}[%*] %F{green}%n@%m %F{blue}%~ \$ %f\$([[ \$__vi_mode == 'vicmd' ]] && echo %u)%{$ftcs_command_start%}"
}
